...
const POLL_INTERVAL = 15_000 // 15 seconds for private dashboard
...
export function MonitorDetailLive({
  ...
  // Also refresh immediately when RunNow completes via a custom event
  React.useEffect(() => {
    const handler = () => {
      fetch(`/api/monitors/${monitor.id}`, { cache: "no-store" })
        .then((r) => r.json())
        .then((data) => {
          setMonitor(data.monitor)
          setRuns(data.runs)
    ...
    window.addEventListener("probe:run-complete", handler)
    return () => window.removeEventListener("probe:run-complete", handler)
  }, [monitor.id])
  ...
  return (
    <>
      {/* Header actions */}
      <div className="flex flex-wrap items-center justify-between gap-4 px-4 pt-4 md:px-8">
        <div>
          <div className="font-mono text-xs text-muted-foreground">
            ~ / monitors / {monitor.slug}
          </div>
          <h1 className="mt-1 text-2xl font-semibold tracking-tight">{monitor.name}</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            {monitor.description ?? `Probing ${step?.url ?? "an endpoint"} every ${monitor.schedule}.`}
          </p>
        </div>
      ...
      <div className="space-y-6 px-4 py-6 md:px-8 md:py-8">
        ...
        <div className="flex items-center gap-2 text-[11px] font-mono text-muted-foreground">
          ...
          live · refreshed {new Date(lastRefresh).toLocaleTimeString()}
        </div>
